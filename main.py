...
def procesar_excel(excel_file, output_pdf, imagen):
    ...
    df_filtrado = df[
        ...
        (df['Impuesto'].str.contains('ganancias|bienes personales', case=False, na=False))
    ]

    # Eliminar las columnas innecesarias
    df_filtrado = df_filtrado.drop(['Concepto / Subconcepto', 'Int. resarcitorios', 'Int. punitorios'], axis=1)
    ...
    # Insertar filas adicionales para dar espacio a la imagen
    ws.insert_rows(1, amount=7)  # Inserta 5 filas para la imagen
    
    # Agregar una imagen encima del encabezado
    ...
    img.width = total_width * 7  # Ajustar el multiplicador según sea necesario
    img.height = 120  # Ajustar la altura de la imagen
    ws.add_image(img, 'A1')  # Colocar la imagen en la celda A1

    # Ajustar el ancho de las columnas automáticamente para el contenido
    for col in ws.columns:
        ...
        for cell in col:
            try:
                ...
                pass
        adjusted_width = (max_length + 2) * 1.2  # Ajustar un poco más el ancho
    ...
    for cell in ws[8]:  # El encabezado está ahora en la fila 6
    ...
    # Exportar Excel a PDF ajustado a una sola página
    app = xw.App(visible=False)
    wb_xw = app.books.open(excel_file)
    wb_xw.sheets[0].api.PageSetup.Zoom = False  # Desactivar zoom
    wb_xw.sheets[0].api.PageSetup.FitToPagesWide = 1  # Ajustar a una página de ancho
    wb_xw.sheets[0].api.PageSetup.FitToPagesTall = 1  # Ajustar a una página de alto
    wb_xw.to_pdf(output_pdf)  # Exportar a PDF
    wb_xw.close()
    app.quit()
